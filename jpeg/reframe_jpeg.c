...
GF_Err jpeg_configure_pid(GF_Filter *filter, GF_FilterPid *pid, Bool is_remove)
{
...
Bool jpeg_process_event(GF_Filter *filter, const GF_FilterEvent *evt)
{
	GF_FilterEvent fevt;
	GF_ReframeJpegCtx *ctx = gf_filter_get_udta(filter);
...
GF_Err jpeg_process(GF_Filter *filter)
{
	GF_ReframeJpegCtx *ctx = gf_filter_get_udta(filter);
